...
namespace Teeleh.WApi.Controllers
{
    ...
    public class UsersController : ApiController
    {
        ...
        /// </returns>
        [HttpPost]
        [Route("api/users/phone/validate")]
        ...
        /// <summary>
        /// This api set a preferred location (province, city) for a user specified with given session info.
        /// </summary>
        /// <returns>
        /// 200 : New Location Replaced  |
        /// 401 : User Not Found (Wrong Session Info) |
        /// 400 : Bad Request 
        /// </returns>
        [HttpPost]
        [Route("api/users/location/new")]
        public async Task<IHttpActionResult> SetPreferredLocation(LocationPairDto locationDto)
        {
            if (ModelState.IsValid)
            {
                var sessionInDb = await
                    db.Sessions.SingleOrDefaultAsync(QueryHelper.GetUserValidationQuery(locationDto.Session));
                if (sessionInDb != null)
                {
                    var user = sessionInDb.User;
                    user.SetUserLocation(locationDto.ProvinceId, locationDto.CityId);
                    return Ok();
                }
        ...
        /// </returns>
        [HttpPost]
        [Route("api/users/email/validate")]
